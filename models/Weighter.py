...
class Weighter(BaseEstimator):
    ...
    def fit(self, dataframe, target, scaled=False, filename='weights'):
        """
        ...
        ----------
            ...
            filename: str
                ...
                and the pickle with the classifier object)
                default: "weights"
        """
        if self.label_type_ == 'w-dl':
            y = dataframe[target].apply(lambda x: 1 if x > 0 else 0)
        ...
        self.clf_ = LinearSVC(fit_intercept=True, random_state=self.random_state_)

        f1_score = np.mean(cross_val_score(self.clf_, X, y, cv=5, scoring='f1_weighted'))
        ...
        for feature, weight in zip(self.feature_names_, self.weights_):
            features_and_weights.append({'feature_name': feature, 'weight': weight})
